//Based on kvjs by HeyPuter (https://github.com/HeyPuter/kv.js.git)
...
class kvjs {
    constructor(snapshotFile = './data.snapshot.json') {
        ...
        // 初始化定期快照循环
        this._initSnapshotLoop(SNAPSHOT_INTERVAL);

    }

    /**
     * 创建当前缓存状态的快照
     * @returns {Object} 包含所有键值对和过期时间的快照对象
     */
    async snapshot() {
        const localfile = this.snapshotFile;
        const snapshot = {
            data: [],
            expireTimes: []
        };
        ...
        // 保存所有键值对
        snapshot.data = Array.from(this.store.entries());

        // 保存所有过期时间
        snapshot.expireTimes = Array.from(this.expireTimes.entries());

        // 如果指定了本地文件路径,则将快照保存到文件
        if (localfile) {
            const fs = require('fs').promises;
            try {
                await fs.writeFile(localfile, JSON.stringify(snapshot, null, 2),'utf-8');
                console.log(`快照已保存到文件 ${localfile}`);
                return snapshot;
            } catch (err) {
                throw new Error(`保存快照到文件 ${localfile} 失败: ${err.message}`);
            }
        }
    }
    ...
    /**
     * 从快照恢复缓存状态
     * @param {string} localfile - 要恢复的快照文件路径
     */
    restoreFromSnapshot() {
        const localfile = this.snapshotFile;
        const fs = require('fs');
        ...
        try {
            ...
            // 恢复过期时间
            for (const [key, expireTime] of snapshot.expireTimes) {
            ...
            // throw new Error(`从文件 ${localfile} 恢复快照失败: ${err.message}`);
            console.error(`从文件 ${localfile} 恢复快照失败: ${err.message}`);
        }
    }
    ...
    */
    ...
    /**
     * 初始化定期快照循环
     * @param {number} snapshotIntervalMs - 快照间隔时间(毫秒)
     * @param {string} localfile - 快照保存路径
     */
    _initSnapshotLoop(snapshotIntervalMs = SNAPSHOT_INTERVAL, localfile = './data.snapshot.json') {
        // 如果已存在快照循环则先清除
        ...
        // 创建新的快照循环
        this.snapshotLoop = setInterval(async () => {
            try {
                await this.snapshot(localfile);
                console.log(`定期快照已创建并保存到 ${localfile}`);
            } catch (err) {
                console.error(`创建定期快照失败: ${err.message}`);
            }
        }, snapshotIntervalMs);
    }
